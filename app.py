...
def consultar_encomienda():
    ...
    try:
        ...
        app.logger.error(f"Error en consultar_encomienda: {e}")
        return jsonify({'success': False, 'message': 'Error al consultar la base de datos'})
...
def activar_entrada():
    ...
    try:
        ...
        if estacionamiento_asignado:
            ...
            conn.close()
            return jsonify({'success': True, 'garaje': garaje_id, 'confirmar': True})
        else:
        ...
        app.logger.error(f"Error en activar_entrada: {e}")
